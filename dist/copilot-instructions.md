# copilot-instructions.md — secure-code skills (minimal)
...
security-sensitive code in this GitHub Copilot project. For dependencies, secrets, authentication,
...
1. Prefer the MCP server when available. Connect to `skills-mcp`
   ...
   - `check_dependency(package, ecosystem, version?)` and
     `check_typosquat(package, ecosystem?)` before adding a new
     dependency.
   - `scan_secrets(text | file_path)` or `check_secret_pattern(text)`
     before committing any string that might be a credential.
   - `map_compliance_control(skill_id | query, framework?)` to
     cite the matching SOC 2 / HIPAA / PCI DSS control.
   - `get_sigma_rule(rule_id | query, category?)` for detection
     logic from `rules/`.
   - `version_status()` to confirm data version and signature
