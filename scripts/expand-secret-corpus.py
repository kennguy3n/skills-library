...
`source = "derived-expand-corpus"` before re-emitting. Curated fixtures
(no `source`, or `source != "derived-expand-corpus"`) are preserved
verbatim.
...
def make_tp_fixtures() -> list[dict]:
    ...
    for prefix in ("xoxb-", "xoxa-", "xoxp-", "xoxs-"):
        for _ in range(3):
            tok = f"{prefix}{secrets.randbelow(10**12)}-{secrets.randbelow(10**12)}-{b62(24)}"
            out.append(
                {
    ...
    # Discord webhook (snowflake + 70-char token).
    for _ in range(8):
        snowflake = "".join(secrets.choice(string.digits) for _ in range(18))
        tok = b62(70)
        url = f"https://discord.com/api/webhooks/{snowflake}/{tok}"
        out.append(
            {
                "id": f"tp-discord-{snowflake[-4:]}",
...
def make_tn_fixtures() -> list[dict]:
    ...
    # Base64-encoded random data with no key shape.
    for _ in range(6):
        blob = secrets.token_urlsafe(32).rstrip("=")
        out.append(
            {
    ...
    # PEM CERTIFICATE blocks (NOT private keys).
    for _ in range(4):
        out.append(
            {
                "id": f"tn-pem-cert-{secrets.token_hex(3)}",
                "text": (
                    "-----BEGIN CERTIFICATE-----\n"
                    f"{b62(64)}\n{b62(64)}\n-----END CERTIFICATE-----"
                ),
