...
The committed `vulnerabilities/osv/` directory is a **stride-sampled subset**
of the upstream OSV archives at `osv-vulnerabilities.storage.googleapis.com`,
generated by `scripts/ingest-osv.py`. It exists as an offline fallback so a
fresh `git clone` can scan without network access. Sample size is controlled
by `--per-ecosystem` (default 500); per-ecosystem advisory counts are
